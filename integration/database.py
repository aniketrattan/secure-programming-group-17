...
class SecureMessagingDB:
    def __init__(self, db_path: str = None):
        if db_path is None:
            # Default to integration folder
            import os
            integration_dir = os.path.dirname(os.path.abspath(__file__))
            self.db_path = os.path.join(integration_dir, "secure_messaging.db")
        else:
    ...
    def create_username(self, user_id: str, username: str) -> None:
        if not _is_uuid_v4(user_id):
            raise ValueError("user_id must be UUID v4")
        canon = self.canonicalize_username(username)
        with sqlite3.connect(self.db_path) as conn:
            ...
            # enforce uniqueness
            row = conn.execute(
                "SELECT 1 FROM usernames WHERE canonical=?", (canon,)
            ).fetchone()
            if row:
                raise ValueError(USERNAME_TAKEN)
            conn.execute(
                "INSERT INTO usernames(username, canonical, user_id) VALUES (?,?,?)",
                (username, canon, user_id),
            )
            # also set display_name in users.meta to the human-entered username for UI fallback
            cur = conn.execute("SELECT meta FROM users WHERE user_id=?", (user_id,))
            mrow = cur.fetchone()
            meta = {}
            if mrow and mrow[0]:
                try:
            ...
            conn.execute(
                "UPDATE users SET meta=? WHERE user_id=?",
                (json.dumps(meta, separators=(",", ":")), user_id),
            )
            conn.commit()

    def get_user_by_username(self, raw_username: str) -> Optional[Dict[str, Any]]:
        canon = self.canonicalize_username(raw_username)
        with sqlite3.connect(self.db_path) as conn:
            cur = conn.execute(
                """
                SELECT u.user_id, u.pubkey, u.meta
                FROM users u
                JOIN usernames un ON un.user_id = u.user_id
                WHERE un.canonical=?
            """,
                (canon,),
            )
    ...
    def init_database(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_usernames_canonical ON usernames(canonical)"
            )
    ...
    def _migrate_users_table(self, conn):
        ...
        if "privkey_store" not in cols:
            conn.execute(
                "ALTER TABLE users ADD COLUMN privkey_store TEXT NOT NULL DEFAULT ''"
            )
            altered = True
        if "pake_password" not in cols:
            conn.execute(
                "ALTER TABLE users ADD COLUMN pake_password TEXT NOT NULL DEFAULT ''"
            )
            altered = True
    ...
    def _initialize_public_channel(self, conn):
        ...
        row = conn.execute(
            "SELECT group_id FROM groups WHERE group_id = 'public'"
        ).fetchone()
        if not row:
    ...
    def set_wrapped_public_key(
        self, member_id: str, wrapped_b64: str, added_at: int
    ) -> None:
        with sqlite3.connect(self.db_path) as conn:
    ...
    def upsert_trusted_server(
        self, server_id: str, pubkey: str, ws_uri: Optional[str] = None
    ) -> None:
    ...
    def load_trusted_mapping(self) -> Dict[str, str]:
        with sqlite3.connect(self.db_path) as conn:
            rows = conn.execute(
                "SELECT server_id, pubkey FROM trusted_servers"
            ).fetchall()
    ...
    def get_user_keys(self, user_id: str) -> Optional[Dict]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            if row:
                return {"pubkey": row[0], "privkey_store": row[1], "version": row[2]}
            return None

    def update_presence(
        self, user_id: str, status: str, session_id: Optional[str] = None
    ) -> bool:
    ...
    def create_group(
        self, group_id: str, creator_id: str, meta: Optional[Dict] = None
    ) -> str:
        meta_json = json.dumps(meta) if meta else None
    ...
    def join_group(
        self, group_id: str, member_id: str, wrapped_key: str, role: str = "member"
    ) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if existing:
                    logger.warning(
                        f"User {member_id} is already a member of group {group_id}"
                    )
                    return False
