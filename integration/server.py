...
class Server():
    ...
    def extract_server_id(self, to_server):
        prefix = "server_"
        ...
        try:
            async for raw in ws:
                ...
                if msg_type == UserAuthType.REGISTER:
                    ...
                    uid = frame["from"]
                    password = frame["payload"]["password"]

                    keys = self.login(user_id=uid, password=password)

                    if keys == None:
                        print(f"[SERVER] Failed login for {uid}")

                        failed_msg = protocol.make_envelope(
                        ...
                        await ws.send(failed_msg)


                        continue


                    # Send public key, protected private key    
                    pubkey, privkey_store = keys['pubkey'], keys['privkey_store']
                    
                    success_msg = protocol.make_envelope(
                ...
                # Handle user hello
                    ...
                    if user_id in tables.local_users:
                        ...
                        pubkey = frame["payload"].get("pubkey")
    ...
    def login(self, user_id: str, password: str):
        ...
        stored_hash = self.db.get_hashed_password(user_id=user_id)

        # Verify if username in database
        if stored_hash == None:
            print("[LOGIN FAIL] User not existed")
            return None
        ...
        try:
            ...
            if ph.verify(stored_hash, password):
                keys = self.db.get_user_keys(user_id=user_id)
                return keys
