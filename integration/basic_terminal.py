...
def display_incoming(frame: dict):
    ...
    if t == "DM":
        ...
        names = payload.get("add", [])
        for name in names:
            display_and_log(colored("system", f"[{ts}] [public] joined: {name}"))
    elif t == "PUBLIC_CHANNEL_UPDATED":
        names = payload.get("remove", [])
        for name in names:
            display_and_log(colored("system", f"[{ts}] [public] left: {name}"))
    elif t == "ERROR":
...
"""
                    ...
                    print(colored("system", f"[{pretty_ts(now_iso())}] Online users: {', '.join(users)}"))
                elif cmd == "/tell":
                    if len(parts) < 3:
                    ...
                    print(colored("dm", f"[{pretty_ts(frame['ts'])}] (DM SENT) to {to}: {text}"))
                    ...
                    print(colored("all", f"[{pretty_ts(frame['ts'])}] (ALL SENT) {USER_ID}: {text}"))
                    log_line(f"SENT_FRAME {json.dumps(frame)}")
                elif cmd == "/file":
                    if len(parts) < 3:
                    ...
                    if not p.exists() or not p.is_file():
                        print(colored("error", f"File not found: {path}"))
                        continue
                    ...
                    print(colored("system", f"[{pretty_ts(frame_offer['ts'])}] (FILE OFFER SENT) to {to}: {filename} ({size} bytes) offer_id={offer_id}"))
                    ...
                    print(colored("system", f"[{pretty_ts(frame_end['ts'])}] (FILE SENT) {filename} -> saved as {sent_copy}"))
                    ...
                    print("Use the simulation commands as documented in /help")
                elif cmd == "/quit":
                    print("Goodbye.")
                    return
                else:
